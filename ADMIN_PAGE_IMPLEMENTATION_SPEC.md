...
문서 상태: 프론트 MVP 실행 명세 / P0 서버 연동 적용
...
- 완료: 관리자 대시보드 실제 API 연동과 화면 모델 변환
- 완료: 매장 승인 목록, 검색/필터, 상세 drawer 실제 API 연동
- 완료: 승인, 보류, 반려 API 연동과 `version` 기반 409 충돌 처리
- 완료: 반려 사유 코드 및 10자 이상 사유 검증
- 완료: 제출 문서 60초 접근 URL 발급 및 미리보기 연결
- 완료: 서버 JWT 역할·권한 정책과 프론트 메뉴/액션 권한 일치
- 완료: 820px 이하 모바일 메뉴·축약 필터와 680px 이하 승인 카드 목록
- 완료: 520px 이하 전체 화면 상세 drawer와 하단 처리 다이얼로그
- 완료: 모바일 터치 영역 최소 44px 및 safe-area 여백 반영
- 유지: 자동화 테스트에서는 외부 서버와 분리된 mock adapter 사용
...
이번 1차 작업은 **내부 운영자가 사용하는 관리자 프론트엔드 MVP를 구현하고 P0 관리자 API와 연결하는 것**이다. 초기 화면 개발은 mock data 기반으로 진행했으며, 2026-06-15에 대시보드와 매장 승인 흐름을 실제 서버 API adapter로 전환했다.

자동화 테스트와 격리 개발 환경에서는 mock adapter를 유지하며, 일반 개발·운영 빌드는 실제 API를 사용한다. 필요할 때만 `REACT_APP_ADMIN_USE_MOCKS=true`로 관리자 mock을 강제할 수 있다.
